...
from src.feature_generation import Features, sbert_encodings_vclaims_pp1, sbert_encodings_training_pp1, \
    ...
    training_data_general, test_data_pp1
...
from src.feature_generation.file_paths.pp2_files import sbert_encodings_training_pp2, \
    ...
    token_number_test_pp2_tsv, test_data_pp2, infersent_encodings_vclaims_pp2, infersent_encodings_vclaims_pp2_tsv, \
    infersent_encodings_training_pp2, infersent_encodings_training_pp2_tsv, infersent_encodings_test_pp2, \
    infersent_encodings_test_pp2_tsv
    ...
    def create_sentence_features(list_of_features, data_set):
        ...
        if Features.sbert.name in list_of_features:
            try:
                ...
                data_encoder.encode(data_set, filename, vclaim_key)
        ...
        if Features.infersent.name in list_of_features: #InferSent - Encodings have to be done for all encodings at once
            try:
                if 'pp1' in data_set:
                    ...
                    dataset_train_pp1 = training_data_general
                    dataset_test_pp1 = test_data_pp1
                    dataset_test_TEST = TEST_test_data
                    ...
                    data_encoder.encode(dataset_vclaims, filename_vclaims, vclaim_key)
                    pd.read_pickle(filename_vclaims).to_csv(filename_vclaims_tsv)
                    data_encoder.encode(dataset_train_pp1, filename_train_pp1, vclaim_key)
                    pd.read_pickle(filename_train_pp1).to_csv(filename_train_pp1_tsv)
                    data_encoder.encode(dataset_test_pp1, filename_test_pp1, vclaim_key)
                    pd.read_pickle(filename_test_pp1).to_csv(filename_test_pp1_tsv)
                    data_encoder.encode(dataset_test_TEST, filename_test_TEST, vclaim_key)
                    pd.read_pickle(filename_test_TEST).to_csv(filename_test_TEST_tsv)
                elif 'pp2' in data_set:
                    dataset_vclaims = v_claims_directory
                    dataset_test_pp2 = test_data_pp2
                    filename_vclaims = infersent_encodings_vclaims_pp2
                    filename_vclaims_tsv = infersent_encodings_vclaims_pp2_tsv
                    filename_test_pp2 = infersent_encodings_test_pp2
                    filename_test_pp2_tsv = infersent_encodings_test_pp2_tsv

                    sentences_for_vocab = []
                    sentences_for_vocab.extend(DataEncoder.get_sentences_to_encode(dataset_vclaims))
                    sentences_for_vocab.extend(DataEncoder.get_sentences_to_encode(dataset_test_pp2))
                    data_encoder = DataEncoder('infer_sent_encoder', 'fast_text_embeddings', 'full_text',
                                               sentences_for_vocab)

                    data_encoder.encode(dataset_vclaims, filename_vclaims, vclaim_key)
                    pd.read_pickle(filename_vclaims).to_csv(filename_vclaims_tsv)
                    data_encoder.encode(dataset_test_pp2, filename_test_pp2, vclaim_key)
                    pd.read_pickle(filename_test_pp2).to_csv(filename_test_pp2_tsv)
            except RuntimeError:
        ...
        if Features.ne.name in list_of_features:
            try:
                ...
                ne_dis.disambiguate_named_entities(data_set, filename, vclaim_key)
                pd.read_pickle(filename).to_csv(filename_tsv)
            except RuntimeError:
        ...
        if Features.main_syms.name in list_of_features:
            try:
                ...
                DataLexemeFinder.find_all_lexemes(data_set, filename, lex_finder, vclaim_key)
                pd.read_pickle(filename).to_csv(filename_tsv)
            except RuntimeError:
        ...
        if Features.subjects.name in list_of_features:
            try:
                ...
                syn_sim_finder.find_all_syntact_info(data_set, filename, vclaim_key)
                pd.read_pickle(filename).to_csv(filename_tsv)
            except RuntimeError:
